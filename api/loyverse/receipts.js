export default async function handler(req, res) {
  ...
  try {
    ...
    let url = `https://api.loyverse.com/v1.0/receipts?limit=${limit}`;
    if (cursor) {
      url += `&cursor=${cursor}`;
    }

    console.log('🧾 Fetching Loyverse receipts:', url);
    ...
    console.log('📡 Loyverse receipts response:', response.status);
