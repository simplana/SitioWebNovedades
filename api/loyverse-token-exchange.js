export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('🔄 Enhanced Loyverse token exchange request:', {
      clientId,
      tokenUrl,
      redirectUri,
      hasCode: !!code,
    ...
    console.log('📡 Loyverse token response status:', tokenResponse.status);
    ...
    console.log('✅ Loyverse token exchange successful');

    // Retornar datos del token (sin registrar información sensible)
    res.status(200).json({
    ...
    console.error('❌ Enhanced token exchange error:', error);
    res.status(500).json({ 
